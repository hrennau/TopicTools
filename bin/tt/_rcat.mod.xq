(:
 :***************************************************************************
 :
 : rcat.mod.xq - functions for creating and resolving resource catalogs
 :
 :***************************************************************************
 :)
...
declare function m:_rcat_csv($dfd as xs:string?, 
    ...
    return
        trace( m:_rcatFromDfd($dfd, $format, $targetFormat, $encoding, $rcatProperties) , 'RCAT: ')
};
    ...
    return
        ...
        if (count($fields) eq 1) then
            ...
            let $dirs := trace( 
                ...
                else replace($dirFiles, '(^.*)/.*', '$1', 's') , 'DIRS: ')
...
(:~
 ...
 : @param optional element with attributes to be copied into the rcat root element 
