    ...
    let $parts := 
        ...
        if (not(matches($text, '^\s*~?\s*\('))) then        
            if (not(contains($text, '/'))) then 
                m:parseNameFilter($text)      (: => 'nameFilter' :)
            else
                m:_parseNameFilterPath($text) (: => 'nameFilterPath' :)
        else    
            m:_parsePathFilterRC($text)
...
(:~
 ...
 :)
declare function m:_parseNameFilterPath($text as xs:string)
        as item()* {
...
declare function m:_parseNameFilterPath($text as xs:string, $atts as attribute()*)
    ...
    let $next :=
        if (not(starts-with($text, '('))) then () else            
            substring($text, 1 + string-length($pathText))[normalize-space(.)]   
    return
        ...
        let $pathTextChopped := replace($pathText, '^\(|\)$', '')
(:        
        ...
        return
            ...
            if (matches($next, '^\S') and not(starts-with($next, '('))) then () else
:)            
        let $parsed :=
            if (not(contains($pathTextChopped, '/'))) then 
                m:_parseNameFilter($pathTextChopped, $atts) 
            else        
...
declare function m:_parseNameFilterPathRC($text as xs:string)
    ...
    let $afterSep :=
        if ($sep) then replace($s, '^/+\s*', '') else $s
    return
...
declare function m:_parsePathFilterRC($text as xs:string)
    ...
    return
        if (not($head)) then ()
            ...
            let $pathEtc := m:_parseNameFilterPath($head, $negFlag)
            return
            ...
            m:_parsePathFilterRC(concat('(', $head, ')'))
};
