(:
 : ===================================================================================
 : util.xqm - various utility functions of the ttools application
 :
 : @version 20150126-1 
 : ===================================================================================
 :)
...
declare function f:getExtensionsModuleURI($directory as xs:string, $toolName as xs:NCName)
    ...
    let $useDir :=        
        ...
        return
            resolve-uri($ttDir, tt:static-base-uri())   
    return
        string-join(($useDir, '_extensions.xqm'), '/')        
};
...
declare function f:getToolNamespace($request as element(), $toolName as xs:string, $toolSettings as element()?)
    ...
    return
        if ($explicitRoot) then
            <namespace 
                func="{if ($explicitFunc) then $explicitFunc else concat($explicitRoot, '/xquery-functions')}"
                error="{if ($explicitError) then $explicitError else concat($explicitRoot, '/error')}"
                struct="{if ($explicitStruct) then $explicitStruct else concat($explicitRoot, '/structure')}"                
            />
 ...
 declare function f:featuresFromFlavor($flavor as xs:string)
    ...
    if (matches($flavor, '^basex(\d\d)', 'i')) then (
        ...
        return (
            ...
            if (contains($additional, 'm')) then 'mongo' else 'mongo-'            
        )
};
